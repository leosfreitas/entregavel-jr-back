...
from use_cases.appraiser.budget.create_budget.create_budget_use_case import CreateBudgetUseCase

router = APIRouter()

create_budget_use_case = CreateBudgetUseCase(BudgetRepository())

@router.post("/user/create-budget", dependencies=[Depends(validade_appraiser_auth_token)])
