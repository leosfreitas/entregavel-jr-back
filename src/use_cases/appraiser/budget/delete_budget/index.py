...
from use_cases.appraiser.budget.delete_budget.delete_budget_use_case import DeleteBudgetUseCase

router = APIRouter()

delete_budget_use_case = DeleteBudgetUseCase(BudgetRepository())

@router.delete("/user/delete-budget/{budget_id}", dependencies=[Depends(validade_appraiser_auth_token)])
